...
def get_contact_info(full_name):
    ...
    print("least_busy_agent_id", least_busy_agent_id)

    # Find from agent list
    least_busy_agent = agents.from_id(least_busy_agent_id)
    print("least_busy_agent", least_busy_agent)

    for i in agents.list:
        print(i)
        print("i.userId", i.userId)
        print("i.portalId", i.portalId)
